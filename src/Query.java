...
public class Query {
	...
	public static void createSchema(String schema_name)
	{
		...
		schema = schema_name;
			...
			try {
				if (Schema.existSCHEMATA(schema_name))
				{
				...
				if (!Schema.isSchemaExist(schema_name))
	...
	public static void useSchema(String schema_name)
	{
		schema=schema_name;
				...
				if (Schema.existSCHEMATA(schema))
				{
	...
	public static void showTables()
		...
		public static void createTable(String table, String[] insertRows) 
		{
			...
			else
			{
				...
				Schema.updateSystemTABLES(schema, table_name);
				Schema.updateSystemCOLUMNS(schema, table_name, column_name, column_type, is_nullable, column_key); 
			}
		}
		...
		if (schema.equals(""))
			...
			else if (!Schema.existInTABLES(schema, table_name))
			{
			...
			else
			{
				...
				if (Schema.InsertTypeCheck(schema, table_name, temp))
				{
					
				
					Schema.incrementSystemTABLE_ROW(schema, table_name);
					
						
					Schema.insertIntoTable(schema, table_name, temp);
				}
			}
		}
	...
	public static void select(String[] col,String table, String cond)
		{
			...
			condition = cond;
			try {
				Schema.SelectFromTable(schema, table_name, condition);
			}
			catch(Exception e) {
				System.out.println(e);
			}
	...
	String deleteTableName = schema + "." + table_name + ".tbl";
			if (Schema.dropTable(schema, table_name))
			{
